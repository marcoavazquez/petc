...
$conesc=mysql_query("select * from tescuelas where sClaveEscuela like '".$clave."'");
...
<script>
function inabilitar(){
	document.fordir.guardargen.disabled=true;
	
	}
function habilitar(){
	var calle='<?php echo $esc['sCalle']; ?>';
	var num='<?php echo $esc['sNumero']; ?>';
	var col='<?php echo $esc['sColonia']; ?>';
	var cp='<?php echo $esc['nCP']; ?>';
	var tel='<?php echo $esc['sTelefono']; ?>';
	var cor='<?php echo $esc['sEmail']; ?>';
	if(document.fordir.calle.value != calle || document.fordir.numero.value != num || document.fordir.colonia.value != col || document.fordir.cp.value != cp || document.fordir.tel.value != tel || document.fordir.email.value != cor){
		document.fordir.guardargen.disabled=false;
		}
		else{
			document.fordir.guardargen.disabled=true;
			}
...
function advertencia(x){
	if(document.fordir.calle.value.length != 0 || document.fordir.numero.value.length != 0 || document.fordir.colonia.value.length != 0 || document.fordir.cp.value.length != 0 || document.fordir.tel.value.length != 0 || document.fordir.email.value.length != 0 || document.formalum.g[1].value.length != 0 || document.formalum.g[2].value.length != 0 || document.formalum.g[3].value.length != 0 || document.formalum.g[4].value.length != 0 || document.formalum.g[5].value.length != 0 ||document.formalum.g[6].value.length != 0 || document.formalum.a[1].value.length != 0 || document.formalum.a[2].value.length != 0 || document.formalum.a[3].value.length != 0 || document.formalum.a[4].value.length != 0 || document.formalum.a[5].value.length != 0 || document.formalum.a[6].value.length != 0 || document.formmat.archivo.value.length != 0 || document.forcap.capacitacion.value.length!=0 || document.forcap.fechacap.value.length!=0 || document.forcap2.nuevaCap.value.length!=0 || document.foradyd.rfc.value.length!=0 || document.forayd.pst.value.length!=0 || document.forayd.ciclod.value.length!=0 || document.forayd.pst.nombres.length!=0 || document.forayd.appat.value.length!=0 || document.forayd.apmat.value.length!=0 || document.forayd.pst.value.length!=0 || document.forayd.fechaIngreso.value.length!=0 || document.forayd.fechaNac.value.length!=0 || document.forayd.plaza.value.length!=0 || document.forayd.nivEstudio.value.length!=0 ||document.foraesp.rfcesp.value.length!=0 || document.foraesp.cicloesp.value.length!=0 || document.foraesp.nombesp.value.length!=0 || document.foraesp.appatesp.value.length!=0 || document.foraesp.apmatesp.value.length!=0 || document.foraesp.especialidad.value.length!=0 || document.foraesp.horaesp.value.length!=0 || document.foraesp.fechaIngresoEsp.value.length!=0 || document.foraesp.fechaNacEsp.value.length!=0){
		if(confirm(String.fromCharCode(191)+"Seguro que quiere abandonar la página sin guardar?")){
			location.href=x;
			}else{
				return false;
				}
...
function eliminar2(p){
	if(confirm(String.fromCharCode(191)+"Seguro que quiere eliminar este dato?")){
		location.href="eliminar_alum.php?id="+p+"&clave=<?php echo $clave; ?>";
		}else{
			return false;
			}
	}	
function eliminarApEc(ae){
	if(confirm(String.fromCharCode(191)+"Seguro que desea eliminar este grupo datos?")){
		location.href="guardarCambios.php?idap="+ae+"&clavesc=<?php echo $clave ?>&formag=delapec";
		}else{
			return false;
		}
	}
...
<div id="ContenedorMenuNavegacion">
    	<ul id="MenuNavegacion">
			<li><a onclick="return advertencia('petc.php');" href="petc.php"><strong>Admin. Escuelas</strong></a></li>
            <li><a onclick="return advertencia ('admSist/');" href="admSist/" >Admin. Sistema</a></li>
            <li><a onclick="return advertencia('admCoord/');" href="admCoord/">Admin. Coordinaci&oacute;n</a>
            <li><a onclick="return advertencia('admUsr/')" href="admUsr/" >Admin. Usuarios</a></li>
			<li><a onclick="return advertencia('salir.php');" href="salir.php" title="Salir" style="color:#550000;" onClick="return salir()">Salir</a></li>
         </ul>
          <ul id="MenuUbicacion">
			<li>
...
<?php
   ...
   //*****************************************DATOS DE UBICACIÃ“N**********************//
    ...
    <table><tr>
        ...
        <td><b>Calle:</b><input onkeyup="return habilitar();" type="text" name="calle" id="calle" value='<?php echo $ubicacion["sCalle"]; ?>' />
  
		</td>
        <td>  
        <b>No.:</b><input onkeyup="return habilitar();" type="text" name="numero" id="numero" value='<?php echo $ubicacion['sNumero']; ?>' size="7" />
  
  		</td>
        <td>
        <b>Colonia:</b><input onkeyup="return habilitar();" type="text" name="colonia" id="colonia" value="<?php echo $ubicacion['sColonia']; ?>" />

        </td>
        <td>
        <strong>C.P.</strong><input onkeyup="return habilitar();" type='text' name='cp' id="cp" value='<?php echo $ubicacion['nCP']; ?>' size="6"/>
        <!-- <validanguage target="cp" mode="allow" expression="numeric -()" required="true" showAlert="true" maxlength="5" errorMsg="Campo CP vacío"> -->
          </td></tr>
          <tr>
          <td colspan="2">
        <strong>Tel&eacute;fono:</strong><input onkeyup="return habilitar();" type='text' name='tel' id="tel"  value='<?php echo $ubicacion['sTelefono']; ?>' />
	<!-- <validanguage target="tel" mode="allow" onkeyup="validanguage.format('(xxx)x-xx-xx-xx', '-() ', '.')" expression="numeric -()" required="true" showAlert="true" maxlength="15" errorMsg="Campo Tel vacío"> -->
         </td>
         <td colspan="2">
        <strong>Correo electr&oacute;nico:</strong><input onkeyup="return habilitar();" type='text' name='email' id="email" value='<?php echo $ubicacion['sEmail']; ?>' /><br />
        </td></tr>
        <tr><td colspan="4"><b id="errorDom" style="color:#FF0000"></b></td></tr>
        </table>
        <input type='submit' name="guardargen" value=' Guardar ' style='float:right' onclick="return validarDir()" /><br />
	</form>
	</span>
</div>
<!--//**************************ALUMNOS*****************************************************************-->
	...
	<br /><br />
	<?php
	$consdifcic=mysql_query("select * from tAlumnos where sClaveEscuelaAl like '$clave' order by sCicloAl desc");
	if($difciclos=mysql_fetch_array($consdifcic)){
        do{
            $consal=mysql_query("select * from tNum_alum na 
                                 where idNumAlum='".$difciclos['idAlumnos']."' order by nGrado asc");
            if($grupos = mysql_fetch_array($consal)){
                ...
                do{
                    ...
                    echo "<tr><td> <a href='#' onclick='return eliminar2(".$difciclos['idAlumnos'].");'>Eliminar</a></td></tr>";
					echo "</table>";
            }
...
?>
	...
	<input type='submit' name="guardaralum" value=' Actualizar ' style='float:right' onclick="return checaal()" /><br />
	<br />
...
<!--//********************************/APOYOS EOCNOMIMOCOS*********************************************************************//-->	
	<div style='border:2px solid #777; padding:10px; margin:5px; background-color:#DDDDDD'>
	<a href="#apyecn" onclick="mostrarocultar('apyecn')" ><b>Apoyos Econ&oacute;micos</b></a>
	...
	<input type="hidden" name="formag" value="fper" />
		...
		<?php   ///////DIRECTOR////
			...
			$consdir=mysql_query("select rh.sRFC,rh.sNombreRH,rh.sApellidoPaternoRH,rh.sApellidoMaternoRH 
                    ...
                    where rh.sRFC=rhe.sRFCrhc and rhe.sClaveEscuelaRhc like '$clave' and rh.nPuesto=2 and rhe.nEstado=1");
      ...
      $conapdir=mysql_query("select idApoyosEcon,nCiclo,nBrutoMensual,nIsrMensual,sRFCae,nFechaInicio,nFechaFinal 
   		...
   		if($apoydir=mysql_fetch_array($conapdir)){
			...
			do{
				...
				echo "<td><img src='imagen/cerrar.png' onclick='return eliminarApEc(".$apoydir['idApoyosEcon'].")' /></td></tr>";
        ...
        <?php
  		...
  		//////////////////DOCENTES//////////////////////////////////////
			$conspers=mysql_query("select rh.sRFC,rh.sNombreRH,rh.sApellidoPaternoRH,rh.sApellidoMaternoRH 
                    from trecursos_humanos rh,trecursos_hum_ciclo rhe 
                    where rh.sRFC=rhe.sRFCrhc and rhe.sClaveEscuelaRhc like '$clave' and rh.nPuesto=1 and rhe.nEstado=1");
			...
			do{
				...
				echo "<td><input type='text' value='"
                        .($apoydoc['nBrutoMensual']-$apoydoc['nIsrMensual'])*($apoydoc['nFechaFinal']-$apoydoc['nFechaInicio']+1)."'
							 disabled='disabled' size='6'/></td>";
				echo "<td><img src='imagen/cerrar.png'  onclick='return eliminarApEc(".$apoydoc['idApoyosEcon'].")'/></td></tr>";
    ...
    <?php		/////////////////////////////ESPECIALISTAS//////////////////////////////////////////////////////////////////////////////////////////////////	
			$consesp=mysql_query("select rh.sRFC,rh.sNombreRH,rh.sApellidoPaternoRH,rh.sApellidoMaternoRH 
                    from trecursos_humanos rh,trecursos_hum_ciclo rhe 
                    where rh.sRFC=rhe.sRFCrhc and rhe.sClaveEscuelaRhc like '$clave' and rh.nPuesto=3 and rhe.nEstado=1");
      ...
      $conape=mysql_query("select idApoyosEcon,nCiclo,nBrutoMensual,nIsrMensual,sRFCae,nFechaInicio,nFechaFinal 
   		...
   		if($apoye=mysql_fetch_array($conape)){
			do{
				...
				echo "<td><img src='imagen/cerrar.png'  onclick='return eliminarApEc(".$apoye['idApoyosEcon'].")' /></td></tr>";
	...
	<a href="#capac" onclick="mostrarocultar('capac')" ><b>Capacitaciones</b></a>
...
<!--//*************************************************AÑADIR PERSONAL A ESCUELA **********************************************************//-->
	<div style='border:2px solid #777; padding:10px; margin:5px; background-color:#dadada'>
	<a href="#agperyesp" onclick="mostrarocultar('agperyesp')" ><b>A&ntilde;adir Personal a Escuela</b></a>
...
<table style="border:1px solid #888; padding:5px"> 
	<tr><td align="right">RFC:</td><td><input type="text" name="rfc"   id="rfc" size="13" onkeyup="cambiarMay(this.id)"/>
		...
		<select name="pst">
            ...
            <?php
				$consExsDir=mysql_query("select * from tRecursos_hum_ciclo where nPuestoRhc=2 and sClaveEscuelaRhc like '$clave'");
				if(mysql_num_rows($consExsDir)==0){
			?>
...
<div style='border:2px solid #777; padding:10px; margin:5px; background-color:#dddddd'>
	<a href="#bajpers" onclick="mostrarocultar('bajpers')" ><b>Baja de Personal</b></a>
...
$ciclo=mysql_fetch_array($cocic);
	...
	$consdir=mysql_query("select rh.sRFC,rh.sNombreRH,rh.sApellidoPaternoRH,rh.sApellidoMaternoRH 
				...
				rh.nPuesto=2 and rhe.nEstado=1");
	if($direc=mysql_fetch_array($consdir)){
		echo "<strong>Director:&nbsp;</strong><br /><strong style='color:#AA0000'>"
		.$direc["sNombreRH"]."&nbsp;".$direc["sApellidoPaternoRH"]."&nbsp;".$direc["sApellidoMaternoRH"]."</strong>&nbsp;&nbsp;";
	...
	$conspers=mysql_query("select rh.sRFC,rh.sNombreRH,rh.sApellidoPaternoRH,rh.sApellidoMaternoRH 
                            from tRecursos_humanos rh,trecursos_hum_ciclo rhe   
                            where rh.sRFC=rhe.sRFCrhc and rhe.sClaveEscuelaRhc='$clave' and rh.nPuesto=1 and rhe.nEstado=1");
	...
	$consesp=mysql_query("select rh.sRFC,rh.sNombreRH,rh.sApellidoPaternoRH,rh.sApellidoMaternoRH 
                            from tRecursos_humanos rh,trecursos_hum_ciclo rhe   
                            where rh.sRFC=rhe.sRFCrhc and rhe.sClaveEscuelaRhc='$clave' and rh.nPuesto=3 and rhe.nEstado=1");
	$numesp=mysql_num_rows($consesp);
	if ($esp = mysql_fetch_array($consesp)){
